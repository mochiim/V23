...
class ElementAbundance:
    ...
    def differentials_f(self, logT, Y):
        ...
        Omega_r0 = (8 * np.pi**3 * self.G) / (90 * self.H0**2) \
                    * (self.k * self.T0)**4 / (self.hbar**3 * self.c**5) \
    ...
    def task_f(self):
        ...
        T_i = 1e11
        Yn_Ti = (1 + np.exp( (self.m_n - self.m_p) * self.c**2 / self.k * T_i ))**(-1)
        Yp_Ti = 1 - Yn_Ti

        solve = int.solve_ivp(self.differentials_f, t_span=[np.log(100e9), np.log(0.1e9)],\
        ...
        plt.plot(T, 1/(1 + np.exp((self.m_n-self.m_p)*self.c**2/(self.k*T))),\
                 linestyle="dotted", color="tab:orange")
        plt.plot(T, 1 - 1/(1 + np.exp((self.m_n-self.m_p)*self.c**2/(self.k*T))), \
    ...
    def task_h(self):
        ...
        Yn_Ti = (1 + np.exp( (self.m_n - self.m_p) \
                            * self.c**2 / self.k * T_i ) )**(-1)
        Yp_Ti = 1 - Yn_Ti
        YD = 0

        solve = int.solve_ivp(self.differentials_h, t_span=[np.log(1e11), np.log(0.1e9)],\
    ...
    def differentials_i(self, logT, Y):
        ...
        # 2)
        p_D, lambda_He3 = self.reactionb2(T)
        ...
        # 3)
        n_D, lambda_T = self.reactionb3(T)
        ...
        # 4)
        n_He3, lambda_pT = self.reactionb4(T)
        ...
        # 5)
        p_T, lambda_He4 = self.reactionb5(T)
        ...
        # 6)
        n_He3, lambda_He4 = self.reactionb6(T)
        ...
        # 7)
        D_D, lambda_nHe3 = self.reactionb7(T)
        ...
        # 8)
        D_D, lambda_pT = self.reactionb8(T)
        ...
        # 11)
        D_T, lambda_He4n = self.reactionb11(T)
        ...
        # 15)
        He3_T, lambda_He4D = self.reactionb15(T)
        ...
        # 16)
        He3_He4, lambda_Be7 = self.reactionb16(T)
        ...
        # 17)
        T_He4, lambda_Li7 = self.reactionb17(T)
        ...
        # 18)
        n_Be7, lambda_pLi7 = self.reactionb18(T)
        ...
        # 20)
        p_Li7, lambda_2He4 = self.reactionb20(T)
        ...
        # 21)
        n_Be7, lambda_2He4 = self.reactionb21(T)
    ...
    def task_i(self):
        ...
        solve = int.solve_ivp(self.differentials_i, t_span=[np.log(T_i), np.log(0.01e9)],\

...
def _taskg(plot = False, save = False):
    ...
    # finding phi_end
    tau_end = tau[np.where(epsilon >= 1)][0]
...
def _taskj(plot = False, save = False):
    h, tau, psi, dpsi, ln_a_ai, p_rho_c = _solver()
    phi = psi * E_P
    N_tot, phi_end = _taskg(plot = False, save = False)
    epsilon = _epsilon(phi)
    idx = epsilon <= 1
...
def _taskk(plot = False, save = False):
    h, tau, psi, dpsi, ln_a_ai, p_rho_c = _solver()
    N_tot, phi_end = _taskg(plot = False, save = False)
    N = np.linspace(50, 60, 100)
    phi = np.sqrt(N * E_P **2 / (2 * np.pi) + phi_end ** 2 )
    epsilon = _epsilon(phi)

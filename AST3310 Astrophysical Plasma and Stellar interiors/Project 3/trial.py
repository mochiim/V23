...
class Hydrodynamics:
    ...
    def initialise(self, Gauss = False):
        ...
        self.T[-1, :] = self.T_sun - self.mu * self.m_u * self.g * nabla * self.dy / self.k
        self.P[-1, :] = self.P_sun * (self.T / self.T_sun) ** (1 / nabla)
        self.e[-1, :] = 3/2 * self.P
        self.rho[-1, :] = self.P * self.mu * self.m_u / ( self.k * self.T)
...
if __name__ == '__main__':
    ...
    # Folder: FVis_output_2023-05-25_10-54, flip orientation

    #vis.animate_2D("T", folder = "FVis_output_2023-05-25_10-54")

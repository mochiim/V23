...
class stellar_modelling:
    ...
    def _computation(self):
        ...
        while radius[i] > 0 and mass[i] > 0 and luminosity[i] > 0:
            ...
            """
            r_new, P_new, L_new, T_new, M_new, rho_new, nabla_stable, nabla_star = self._integration(mass[i], radius[i], pressure[i], luminosity[i], temperature[i])
            radius.append(r_new)
            ...
            mass.append(M_new)
            density.append(rho)
            nabla_stable.append(nabla_stable)
            nabla_star.append(nabla_star)

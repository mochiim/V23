...
class stellar_modelling:
    ...
    def readfile(self):
        ...
        data_epsilon = np.loadtxt("/Users/rebeccanguyen/Documents/GitHub/V23/AST3310 Astrophysical Plasma and Stellar interiors/epsilon.txt", dtype = np.str)
        # extracting first row and removing first element
        logR_opacity = data_opacity[0, 1:]
        logR_epsilon = data_epsilon[0, 1:]
        self.logR_opacity = np.asarray(logR_opacity, dtype = float)
        self.logR_epsilon = np.asarray(logR_epsilon, dtype = float)

        # extracting first column and removing first element
        logT_opacity = data_opacity[1:, 0];
        logT_epsilon = data_epsilon[1:, 0];
        self.logT_opacity = np.asarray(logT_opacity, dtype = float)
        self.logT_epsilon = np.asarray(logT_epsilon, dtype = float)

        # extracing the remaining matrix
        logkappa_opacity = data_opacity[1:, 1:]
        logepsilon = data_epsilon[1:, 1:]
        self.logkappa_opacity = np.asarray(logkappa_opacity, dtype = float)
        self.logepsilon = np.asarray(logepsilon, dtype = float)

        # interpolation of opacity values (extrapolated for values outside the bounds)
        # calling RectBivariateSpline outputs an array
        self.polation_opacity = RectBivariateSpline(self.logT_opacity, self.logR_opacity, self.logkappa_opacity)
        self.polation_epsilon = RectBivariateSpline(self.logT_epsilon, self.logR_epsilon, self.logepsilon)
    ...
    def _sanity_check_opacity(self):
        ...
        # computed values
        logkappa_cgs_computed = np.array([self.polation_opacity.ev(logT[i], logR[i])[0][0] for i in range(len(logT))])
    ...
    def _sanity_check_epsilon(self):
        """
        Sanity check of interpolation of opacity values
        """
        logT = [3.750, 3.755]
        ...
        # expected values
        logepsilon_cgs_expected = [-87.995, -87.267]
        epsilon_SI_expected     = [1.012e-92, 5.401e-92]

        # computed values
        logepsilon_cgs_computed = np.array([self.polation_epsilon(logT[i], logR[i])[0][0] for i in range(len(logT))])
        epsilon_SI_computed = 10**logepsilon_cgs_computed*.1

        table = pd.DataFrame({"logε (cgs), exp"      :  logepsilon_cgs_expected,
                              "logε (cgs), com"      :  logepsilon_cgs_computed,
                              "ε (SI), exp"          :  epsilon_SI_expected,
                              "ε (SI), com"          :  epsilon_SI_computed})
